...
const routePurchase = require('../routes/purchase');
...
server._setup = () => {
  ...
  mongoose.connect(process.env.DB_MAIN_CONNECTION,
...
server.init = () => {
  ...
  // execute mining cycle immediately
  MiningCycle(false);
  // mining cycle run every 6 hrs
  setInterval(() => MiningCycle(false),1000 * 60 * 60 * 6);
  //setInterval(() => MiningCycle(false),1000 * 5);
};
