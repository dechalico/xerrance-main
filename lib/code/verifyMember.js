...
function updateAccountSummary(accountSummaryId,accountReferralCodes){
  AccountSummary.findById(accountSummaryId,(err,accountSummary) => {
    if(!err && accountSummary){
      accountSummary.useReferralCodes = accountReferralCodes.usedReferralCodes.length;
      accountSummary.unUsedReferralCodes = accountReferralCodes.unUsedReferralCodes.length;
      accountSummary.referralCodes = accountReferralCodes.referralCodes.length;
      accountSummary.rightCodes = accountReferralCodes.rightCodes.length;
      accountSummary.leftCodes = accountReferralCodes.leftCodes.length;
      accountSummary.totalPairs = accountReferralCodes.rightCodes.length > accountReferralCodes.leftCodes.length ? 
        accountReferralCodes.leftCodes.length : accountReferralCodes.rightCodes.length;
      
      accountSummary.save(err => {
        if(err){
          console.log('Error when updating account summary');
        }
      });
    } else {
      console.log('Error cannot find account summary');
    }
  });
}