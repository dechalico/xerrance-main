...
const UpdateUplineAccount = require('../code/updateUpline');
...
module.exports = function(res,id,token){
  ...
  Member.findById(id,(err,memberResult) => {
    if(!err && memberResult && !memberResult.isEmailVerified) {
      ...
      if(ownerAccountReferralCodes){
        ...
        if(ownerAccountReferralCodes.directDownlines.length < downlineLimit){
          ...
          ownerAccountReferralCodes.save(err => {
            // update accountSummary of the owner of referralCode and upline accounts
            UpdateAccountSummary(ownerAccount,1,() => UpdateUplineAccount(ownerAccount));
            if(err){
...
function accomodateDownline(ownerAccount,ownerReferralCodes,newAccount,referralCode,res){
  ...
  ownerReferralCodes.save(err => {
    // update the accountSummary of the owner of referralCode and upline accounts
    UpdateAccountSummary(ownerAccount,1,() => UpdateUplineAccount(ownerAccount));
    if(err){
      console.log('Error when updating the direct downlines of the owner');
    }
  ...
  for(let i=0; i < referrals.length; i++){
    try{
      ...
      if(referralAccount){
        ...
        const newAccount = await Account.findById(newAccountId).populate('memberId');
        if(accountReferralCodes){
          const level = {
            level: referrals.length -i,
            account: newAccount._id,
            accountName: newAccount.memberId.firstname + " " + newAccount.memberId.lastname,
...
async function createDefaulAccount(member,referralCode){
  try{
    ...
    const purchase = await Purchase.create({referralCodes: [],upgradeRank: []});
